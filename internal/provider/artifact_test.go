...
// TestPackagedArtifactIdentityBootstrapsServer proves that the identity built by
// the real artifact packaging (the same descriptor the release produces) is
// accepted by NewServer, binding the binary to this manifest.
func TestPackagedArtifactIdentityBootstrapsServer(t *testing.T) {
	manifestBytes := loadManifestBytes(t)
	descriptor, err := artifact.BuildDescriptor("provider-sentry", []byte("stand-in-binary"), manifestBytes)
	if err != nil {
