...
// UpgradeState performs one offline state-schema upgrade step. v0.1 defines only
// state schema version 1, so there is no prior schema to advance from: the
// method fails closed rather than fabricating an upgrade.
