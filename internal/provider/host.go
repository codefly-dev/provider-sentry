...
// use: every Sentry read and every output projection travels through it, so the
// provider never opens a socket and never holds a raw credential, a client-key
// `secret`, or a `dsn.secret`.
...
// originRuleAPI is the single manifest origin rule; every Sentry request is
// admitted against it.
...
func (s *Server) plannedRequest(descriptorID string, origin *providerv0.AdmittedOrigin, pathParams, query map[string]*providerv0.PublicValue) (*providerv0.PlannedRequest, error) {
	...
	for _, purposeID := range descriptor.CredentialPurposes {
		purpose, err := s.credentialPurpose(purposeID)
		if err != nil {
			return nil, err
		}
...
// suppression policy in force when it was produced.
...
// credentialPurpose resolves a manifest credential-purpose id (e.g. "setup") to
// the proto purpose enum via its permitted consumer. The setup credential is a
// management-consumer handle used only to read.
func (s *Server) credentialPurpose(purposeID string) (providerv0.CredentialPurpose, error) {
	for _, purpose := range s.manifest.CredentialPurposes {
		...
		default:
			return providerv0.CredentialPurpose_CREDENTIAL_PURPOSE_UNSPECIFIED, fmt.Errorf("credential purpose %q has an unknown consumer %q", purposeID, purpose.PermittedConsumer)
		}
	}
	return providerv0.CredentialPurpose_CREDENTIAL_PURPOSE_UNSPECIFIED, fmt.Errorf("credential purpose %q is not packaged", purposeID)
}
